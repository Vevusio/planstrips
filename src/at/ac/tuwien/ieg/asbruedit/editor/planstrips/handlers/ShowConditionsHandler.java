...
public class ShowConditionsHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		for (PlanStripsEditor editor : PlanStripsEditorRegistry.instance().all()) {
			ConditionsToggleAdapter toggler = (ConditionsToggleAdapter) editor.getAdapter(ConditionsToggleAdapter.class);
			toggler.setShowConditions(!oldValue);
		}
		return null;
	}

	public static boolean isShowConditions() {
		ICommandService commandService = (ICommandService) PlatformUI.getWorkbench().getActiveWorkbenchWindow().getService(ICommandService.class);

		Command command = commandService.getCommand("at.ac.tuwien.ieg.asbruedit.show-conditions-toggle");
