...
public class PlanLayoutEditPolicy extends PlanStripsLayoutEditPolicy {
	@Override
	protected Command getCreateCommand(CreateRequest request) {
		Command cmd = null;
		// if the plan can take take a child
		if (!AsbruModelHandler.instance().planHasChild(getHost().getModel())) {
		...
		// otherwise we delegate the command to the child
			...
			// (semantically a plan can only have one child as of the writing of
			// this class with asbru version 7.3)
	...
	@Override
	public PlanEditPart getHost() {
		return (PlanEditPart) super.getHost();
	}
}
